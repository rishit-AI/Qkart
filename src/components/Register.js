...
class Register extends React.Component {
  ...
  performAPICall = async () => {
    ...
    if (this.validateResponse(error, res)) return res;
  };
  ...
  validateInput = () => {
    ...
    if (this.state.username.trim().length < 6) {
      message.error("Username should not be less than 6 characters");
      return false;
    }
    if (this.state.username.trim().length > 32) {
      message.error("Username should not be greater than 32 characters");
      return false;
    }
    ...
    if (this.state.password.trim().length < 6) {
      message.error("Password should not be less than 6 characters");
      return false;
    }
    if (this.state.password.trim().length > 32) {
      message.error("Password should not be greater than 32 characters");
      return false;
    }
    if (this.state.password !== this.state.confirmPassword) {
      message.error("Password should match Confirm Password");
      return false;
    }
    return true;
  };
