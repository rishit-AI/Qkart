...
class Checkout extends React.Component {
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
        ...
        const cartamt= this.cartRef.current.calculateTotal();
        message.info("Order is successful");
        localStorage.setItem("balance", this.state.balance-cartamt);
  ...
  order = async () => {
    const cartamt= this.cartRef.current.calculateTotal();
    if(this.state.balance < cartamt)
      message.error("Wallet balance is not sufficient");
    else if( !this.state.addresses.length )
      message.error("No address is selected");
    else
      await this.checkout();
    //console.log(cartamt);
  };
  ...
  async componentDidMount() {
    ...
    if(!cart.length){
      message.error("Cart is empty. Please add to cart first");
      ...
      <>
        ...
        <div className="checkout-container">
          <Row>
            ...
            {/* Display the cart */}
            <Col xs={{ span: 24, order : 1 } } md={6} className="checkout-cart">
              <div>
